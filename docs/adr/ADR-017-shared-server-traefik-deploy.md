...
- **Автоматический деплой вынесен, legacy multi-instance выкат удалён (2026-07-10):** описанный в §Решение п.5 CI/CD-механизм — gated `deploy`-job в `ci.yml` (`INSTANCES`-loop на сервер `87.239.135.154`; секреты `SSH_HOST`/`SSH_USER`/`SSH_PRIVATE_KEY`, впоследствии переименованные в `LEGACY_SSH_*`) и ручной `deploy.yml` — **удалён из репозитория**. `ci.yml` теперь чистый quality-гейт (`quality`/`test`/`build-image`, без деплоя); единственный автоматический деплой репозитория — [`deploy-novirell.yml`](../../.github/workflows/deploy-novirell.yml) (сервер novirell `49.12.189.77`, [ADR-060](ADR-060-novirell-shared-nginx-shared-network-deploy.md)), гейтящийся на зелёном `ci` через `workflow_run`. §Решение п.5 сохранён как исторический (immutability); секреты `LEGACY_SSH_*` больше не используются. Актуальная картина CI/CD — [07-deployment.md §CI/CD (gate)](../07-deployment.md#cicd-gate). Ревизия — [INDEX §Ревизии](INDEX.md#ревизии).
