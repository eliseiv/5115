...
- **Имя — dot, == `/v1/tools`:** `steps[].payload.content[]` с `type=tool_use` отдаёт `name` в доменном dot-формате (`site.write_file`), **дословно равном** `name` соответствующего инструмента в `GET /v1/tools` и `toolName` в `GET /v1/chats/{id}/steps`. (Исторические шаги с удалёнными именами `files.*`/`calendar.*`/`reminders.*` отдаются как есть; при нормализации неизвестное имя не переписывается — `UnknownToolNameError` ловится, [ADR-063 §5](../../adr/ADR-063-remove-client-side-calendar-reminders-files-tools.md).)
...
- **Server-side в toolCalls[] не попадает:** смешанный ход (`site.write_file` + client-side инструмент) → `site.*` исполнен на бэке, в `toolCalls[]` только client-side; continuation собирает `tool_result` обоих перед `messages.create`. **С [ADR-063](../../adr/ADR-063-remove-client-side-calendar-reminders-files-tools.md) реальных client-side инструментов нет** — этот сценарий проверяется тестовым фейковым client-side инструментом (регистрируется фикстурой, не поставляется), а не `files.*`/`calendar.*`/`reminders.*`.
