...
- **`workspaceProjectId` (опц., uuid, session-fixed, [ADR-013](../../adr/ADR-013-workspace-projects-vs-website-builder.md)/[ADR-036](../../adr/ADR-036-workspaces-implementation.md)).** Привязка чата к рабочему пространству. Фиксируется на сессию при создании (как `mode`/`assistantMode`/`model`/`projectId`):
  ...
  - **с `workspaceProjectId`** → валидируется **принадлежность workspace пользователю** (`sub`); чужой/несуществующий → **`404 workspace_not_found`** (изоляция, не раскрывать чужое существование). `workspace.instructions` подмешиваются в system-prompt **после** base assistant_mode prompt ([ADR-012](../../adr/ADR-012-assistant-mode-vs-billing-mode.md)/[ADR-036 §3](../../adr/ADR-036-workspaces-implementation.md)); файлы-знания workspace подаются как контекст первого LLM-вызова хода (document/text → `extracted_text`, image → vision; [ADR-036 §6](../../adr/ADR-036-workspaces-implementation.md), актуально по [ADR-064](../../adr/ADR-064-workspace-files-live-reinjection.md));
  - **`instructions` И файлы — на КАЖДОМ ходе сессии с workspace (turn 0, resume И continuation) ([ADR-064](../../adr/ADR-064-workspace-files-live-reinjection.md), ревизует [ADR-036 §3/§6](../../adr/ADR-036-workspaces-implementation.md) и [ADR-038 §3.2](../../adr/ADR-038-move-chat-to-workspace.md)).** При наличии у сессии `workspace_project_id` orchestrator на **каждом** generation-запросе (turn 0 / resume `/chat/run` / первый вызов `/chat/tool-result`) — **независимо от `ctx.is_new`** — вызывает `context_for_session` (instructions + файлы): `instructions` через helper `_system_prompt_with_workspace` в `system`; файлы-знания (`extracted_text`/vision) инъектируются в последний user-turn **первого** LLM-вызова запроса (`first_turn_attachments`). **Файлы НЕ персистируются** в `chat_steps` (инвариант base64 [ADR-020](../../adr/ADR-020-inline-base64-attachments-mvp.md); текст файлов не утекает в user-facing историю → strip [ADR-042](../../adr/ADR-042-hide-context-block-from-user-facing-history.md) не нужен) и **не** входят в `user_payload_content`. Прежняя формулировка «файлы turn-0-only, реплеятся из истории» — **никогда не была реализована** (прод-баг «файлы видны только на turn 0») и снята.
  - **Resume-сессия:** `workspaceProjectId` берётся из сессии; поле запроса при resume **игнорируется** (не ошибка). И `instructions`, и файлы подаются на каждом ходе через `context_for_session` — включая чаты, **перенесённые** в workspace позже (`PATCH /v1/chats/{id}`, [ADR-038](../../adr/ADR-038-move-chat-to-workspace.md)) и с **отредактированным первым сообщением** ([ADR-040 §4а](../../adr/ADR-040-edit-message-and-regenerate.md)); [Q-038-1](../../99-open-questions.md)/[Q-040-3](../../99-open-questions.md) закрыты. Разведение: вложения запроса ([ADR-020](../../adr/ADR-020-inline-base64-attachments-mvp.md)) — turn-0-only (плейсхолдер персистится); файлы workspace — живой контекст каждый ход, без персиста.
...
- **`editMessageStepId` (опц., uuid, [ADR-040](../../adr/ADR-040-edit-message-and-regenerate.md)) — редактирование отправленного сообщения.** Когда указано — backend **усекает** историю сессии от хода `editMessageStepId` (его user-шаг и **всё, что после**) и генерирует **новый** ход с переданными `message`/`attachments`/`context`. Один атомарный вызов (усечение + новый ход в транзакции запроса), без отдельного endpoint'а.
  ...
  - **Edge — редактирование ПЕРВОГО сообщения чата ([ADR-040 §4а](../../adr/ADR-040-edit-message-and-regenerate.md), файлы актуальны по [ADR-064](../../adr/ADR-064-workspace-files-live-reinjection.md)):** усечение удаляет всю историю, сессия становится пустой, но **существует** → `ctx.is_new = False`. По [ADR-064](../../adr/ADR-064-workspace-files-live-reinjection.md) подача файлов развязана от `ctx.is_new` (живая переинъекция на каждом ходе), поэтому **workspace-файлы И instructions инъектируются** и на отредактированном первом ходе — [Q-040-3](../../99-open-questions.md) закрыт. Прежнее ограничение «файлы turn-0-only, не подаются при `is_new=False`» ([ADR-038 §3.2](../../adr/ADR-038-move-chat-to-workspace.md) вариант a) снято. Инлайн-attachments нового хода ([ADR-020](../../adr/ADR-020-inline-base64-attachments-mvp.md)) подаются как turn-0 нового хода (turn-0-only, без изменений).
...
```json
{
  ...
  "stepId": "uuid | null",
  "assistantMessage": "string (optional, при assistant_message; ТАКЖЕ при tool_call, если Claude выдал текст вместе с tool_use — ADR-024 п.3 / Q-024-1; при наличии quiz — детерминированно null, ADR-062 §3)",
  ...
  "quiz": { "questions": [ { "question": "string", "options": ["..."], "correctIndex": 0, "explanation": "string" } ] },
...
- **`quiz` (опц., [ADR-057](../../adr/ADR-057-study-learn-quiz-contract.md) + редизайн [ADR-062](../../adr/ADR-062-study-learn-quiz-pool.md)):** присутствует только когда `dialogMode=study_learn` и модель вызвала global server-side tool `quiz.generate`; иначе `null`/опущено. **С [ADR-062](../../adr/ADR-062-study-learn-quiz-pool.md) `quiz` — ПУЛ вопросов** (breaking, пред-релиз): `QuizSchema(StrictModel) = { questions: [QuizQuestionSchema] }`, `questions` — **3..10** вопросов; каждый `QuizQuestionSchema` = `question` (≤1000), `options` (2..10, cap на длину), `correctIndex` (валидатор `correctIndex < len(options)` **на каждый** вопрос), `explanation` (≤2000). iOS итерирует `quiz.questions[]`. Результат инструмента (полный пул) аккумулируется в `_generate_loop` → `ChatRunOut.quiz`, **last-wins по пулу** (`quiz_acc[-1]`; при нескольких вызовах за ход побеждает последний полный пул, не объединяется). **Результат считается на клиенте** — `correctIndex`/`explanation` в `quiz.questions[]`, проверка после ответа пользователя **локально**, эндпоинта отправки ответов **НЕТ**, continuation по результатам квиза не вводится (осознанный компромисс: обучение, не экзамен; [ADR-062 §4](../../adr/ADR-062-study-learn-quiz-pool.md)/[ADR-057 §6](../../adr/ADR-057-study-learn-quiz-contract.md)). Совместим с временным чатом. **`assistantMessage` при наличии `quiz` детерминированно `null`** ([ADR-062 §3](../../adr/ADR-062-study-learn-quiz-pool.md)): backend отбрасывает свободный текст модели квиз-хода (устранение дубля вопроса и спойлера ответа — переопределяет [ADR-024](../../adr/ADR-024-history-payload-domain-normalization.md) правило проброса текста на `tool_call` **только** для квиз-хода; история `GET /v1/chats/{id}` не меняется, переопределяется run-проекция); рамку/интро рисует клиент. **`quiz` в ответе всегда валиден** (или `null`): невалидные аргументы квиза **не** роняют ход `422` — backend возвращает `tool_result`-ошибку, модель **исправляет пул в том же ходе** (graceful degrade, как `time.now`/`invalid_timezone` [ADR-026 §6](../../adr/ADR-026-global-server-side-tools-and-time-now.md)) — **all-or-nothing**: любое нарушение (пустой/вне-`3..10` список, у любого вопроса вне-`2..10` варианты, длины, `correctIndex` булев/отрицательный/вне диапазона) делает **весь пул** невалидным ([ADR-062 §7](../../adr/ADR-062-study-learn-quiz-pool.md)/[ADR-057 §3](../../adr/ADR-057-study-learn-quiz-contract.md)). При упорстве модели до `MAX_SERVER_TOOL_ROUNDS` — существующий guard ([ADR-011 §2](../../adr/ADR-011-server-side-tools.md)): `UpstreamError → 502`, без биллинга (патологично). **Strict-нюанс:** OpenAI strict не принимает `minLength`/`maxLength`/`minItems`/`maxItems`/…, поэтому wire-схема очищается (вложенный список вопросов → `$defs`/`$ref`/`items`, переживают очистку; ограничивающие ключи срезаются), а ограничения держатся серверной Pydantic-валидацией + текстом описания; подмодель вопроса **обязана** быть strict (`extra='forbid'` → `additionalProperties:false` + all-required), иначе провайдер даст `400`.
...
- **`assistantMessage` ([Q-024-1](../../99-open-questions.md) Closed = вариант A, [ADR-024 §Decision п.3](../../adr/ADR-024-history-payload-domain-normalization.md)):**
  - `status=assistant_message` — финальный текст Claude. **Исключение (квиз-ход, [ADR-062 §3](../../adr/ADR-062-study-learn-quiz-pool.md)):** если в ответе присутствует `quiz` (не `null`) — `assistantMessage` детерминированно `null` (backend отбрасывает свободный текст модели квиз-хода; квиз обычно сопровождает именно финальный `assistant_message`). Иначе — как и раньше, без изменений.
  - `status=tool_call` — **опционально присутствует**: текст из `text`-блоков **того же** assistant-шага, чей `tool_use` вернулся как `toolCall` (тот шаг, на который указывает `stepId`). Значение = текст/конкатенация `text`-блоков этого шага. Если Claude вернул `tool_use` **без** сопутствующего текста — `assistantMessage = null`/опущено. **Исключение (квиз-ход, [ADR-062 §3](../../adr/ADR-062-study-learn-quiz-pool.md)):** при наличии `quiz` в ответе `assistantMessage` детерминированно `null` даже если у шага был сопутствующий текст (см. поле `quiz`) — переопределяет проброс текста только для квиз-хода. `toolCall` при этом **обязателен** (семантика не меняется); добавление `assistantMessage` аддитивно/обратносовместимо (поле уже опционально-nullable в схеме; новизна — оно теперь может быть НЕ-null при `tool_call`). Backend перестаёт отбрасывать сопутствующий текст (`orchestrator.py:661`) и кладёт его в `assistantMessage`.
...
| quiz.generate | **server** (global, [ADR-057](../../adr/ADR-057-study-learn-quiz-contract.md) + [ADR-062](../../adr/ADR-062-study-learn-quiz-pool.md)) | нет |
