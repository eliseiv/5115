...
- **`preview` без промта экшена ([ADR-065 §3,§7](../../adr/ADR-065-action-prompt-hidden-per-turn-prompt.md)):** `actionPrompt` ([ADR-065](../../adr/ADR-065-action-prompt-hidden-per-turn-prompt.md)) хранится **вне `content[]`** — отдельным ключом `chat_steps.payload.actionPrompt`, а `_preview`/`_text_from_payload` итерируют **только** `payload["content"]` → промт экшена в превью не попадает **by construction** (regex/strip для него не нужен). Edge: ход `message=""` + `actionPrompt` (без вложений) → `content = []` → превью этого шага **пусто** (`null`), как у image-only с `context`. Автозаголовок (`derive_title`) считается только из `message` → промт не утекает и в `title`. Поиск `?q=` идёт по `payload["content"][0]["text"]` → скрытый промт **не ищется** (и не должен).
...
- `payload` — payload шага. **Отдаётся в ДОМЕННОМ виде (нормализация при отдаче, [ADR-024](../../adr/ADR-024-history-payload-domain-normalization.md)), а НЕ в сыром виде хранилища.** Нормализация применяется на границе сериализации ответа (хранение `chat_steps.payload` и реплей в Claude не меняются). Форма зависит от `role`:
  ...
  - **`role="assistant"` content-блоки (`type=text` / `type=tool_use`):**
    - **`tool_use.name`:** underscore → dot (`site_write_file` → `site.write_file`) через `to_domain_tool_name` — совпадает с `/v1/tools` `name`, `/chat/run` `toolCall.name`, `/v1/chats/{id}/steps` `toolName`. Исторические шаги с удалёнными именами (`files_write` и т.п., [ADR-063](../../adr/ADR-063-remove-client-side-calendar-reminders-files-tools.md)) отдаются как есть — `UnknownToolNameError` ловится, имя не переписывается.
  ...
  - **User-шаг без промта экшена ([ADR-065 §3](../../adr/ADR-065-action-prompt-hidden-per-turn-prompt.md)):** если ход был отправлен с `actionPrompt` (промт кнопки-действия, виден модели — не виден пользователю), он персистится **вне `content[]`**, отдельным ключом верхнего уровня `chat_steps.payload.actionPrompt` (для корректного реплея). В истории этот ключ **удаляется при отдаче**: `ChatsService._normalize_payload` обязан `pop("actionPrompt")` на той же deep-копии, где выполняется нормализация [ADR-024](../../adr/ADR-024-history-payload-domain-normalization.md) и срез блока [ADR-042](../../adr/ADR-042-hide-context-block-from-user-facing-history.md) (тот же паттерн, что `providerToolUseId`). Клиент видит **только** `content[]` = видимый текст + плейсхолдеры вложений. **Хранение и реплей (`_build_messages`) не меняются** — модель промт получает (блок синтезируется из ключа при сборке сообщений). **Edge `message=""` + `actionPrompt` — user-шаг с ПУСТЫМ `content` (важно для клиента, [ADR-065 §5.2](../../adr/ADR-065-action-prompt-hidden-per-turn-prompt.md)):** у такого шага `payload.content = []` (пустой массив). Это **первый в системе** валидный user-шаг без единого блока: у attachment-only хода ([ADR-039](../../adr/ADR-039-optional-message-with-attachments.md)) всегда был хотя бы text-плейсхолдер вложения. **Ожидаемое поведение, не дефект данных.** Клиент **обязан** это учитывать: такой шаг рендерить как **пустой user-пузырь нельзя** — рекомендуется не отрисовывать его вовсе (диалог начинается сразу с ответа ассистента) либо показать собственный UI-элемент нажатого чипа **из своего локального состояния**. Сопутствующее: чат, начатый экшеном без текста, до первого ответа ассистента имеет `title = null` (auto-title считается только из `message`) и `preview = null`. **Метка экшена в историю НЕ вводится** — сервер не отдаёт признак «этот ход был экшеном» (это потребовало бы нового ADR); клиент, которому нужна такая метка, ведёт её у себя. **Нормативный инвариант:** сырой `payload` user-шага наружу не отдаётся; любая новая user-facing проекция идёт через `_normalize_payload` либо читает только `content[]`.
