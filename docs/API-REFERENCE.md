...
**Response 200:** сырые байты изображения; `Content-Type` = `image/png`\|`image/jpeg`\|`image/webp`; security-заголовки `X-Content-Type-Options: nosniff`, `Cache-Control: no-store` (образец preview [ADR-010](adr/ADR-010-backend-hosted-preview.md)).
