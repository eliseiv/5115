...
Изображения генерируются server-side инструментом `image.generate` (OpenAI `gpt-image-1`) в ходе `/v1/chat/run`; байты хранятся вне чата (таблица `generated_images`), в ответе генерации приходит `images[].imageId` (только id/`contentType`/`size`, без промпта — [ADR-058 §7](adr/ADR-058-image-generation.md)), а байты фетчатся отдельным endpoint. Генератор — отдельный OpenAI-клиент, **независимый от `LLM_PROVIDER`**: `image.generate` активен на **любом** инстансе, где задан `OPENAI_API_KEY` (включая Anthropic-чат-инстанс с настроенным ключом), и не предлагается там, где ключ не задан ([ADR-058 §3](adr/ADR-058-image-generation.md)).
