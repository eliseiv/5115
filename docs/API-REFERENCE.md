...
| `quiz` | object `{ question, options[], correctIndex, explanation }` \| null, опц. | **Квиз режима Study & Learn** ([ADR-057](adr/ADR-057-study-learn-quiz-contract.md)). Присутствует только при `dialogMode=study_learn`, когда модель сгенерировала квиз; иначе `null`. `options` — 2..10; `correctIndex < len(options)` (проверка ответа на клиенте). **Всегда валиден** (или `null`): невалидный квиз от модели не роняет ход — backend просит модель исправить его в том же ходе (graceful degrade), ход завершается с объяснением и корректным квизом ([ADR-057 §3](adr/ADR-057-study-learn-quiz-contract.md)). |
...
| `quiz.generate` | read | генерация квиза (**только** при `dialogMode=study_learn`, [ADR-057](adr/ADR-057-study-learn-quiz-contract.md)). `strict`-схема args `{ question, options[2..10], correctIndex, explanation }` (все поля required). «Исполнение» = валидация + эхо-возврат; результат поднимается в `ChatResponse.quiz`. Не отдаётся как `tool_call`. |
