...
- **Внешняя docker-сеть:** `mas-net` (НЕ `web` — сети `web` на этом сервере нет). Создана стеком mail-aggregator; `mas-nginx` — её член (адрес `172.18.0.7`). Подсеть — `172.18.0.0/16` (default bridge /16). Наш `api` подключается к `mas-net` через `EDGE_NETWORK=mas-net` (compose-ключ сети `web` → `name: ${EDGE_NETWORK:-web}`), `external: true` — мы её **не создаём**.
...
- **upstream по имени контейнера:** `set $upstream http://novirell-api-1:8000;` + `resolver 127.0.0.11 valid=10s;` (docker embedded DNS резолвит имя контейнера в `mas-net`; `$variable` заставляет nginx резолвить в рантайме — стартует даже при выключенном нашем стеке, переподхватывает новый IP после редеплоя).
- **Имя контейнера `novirell-api-1`** — compose-дефолт для сервиса `api` при project `novirell`. `container_name` НЕ задаётся (см. [§Имя контейнера api](#имя-контейнера-api-novirell)).
...
- **Шаги** (`appleboy/ssh-action`, `script_stop:false`, `set -uo pipefail` без `-e`): `cd /opt/novirell` → `git pull --ff-only` → `compose -p novirell build api migrate` → `run --rm migrate` → `up -d --no-build` → readiness-gate по health `novirell-api-1` (30×2s) → NON-FATAL smoke `https://novirell.shop/healthz` (12×5s). Реальные сбои → `$FAILED` → финальный `exit 1`.
...
6. **Проверка config-рендера (EDGE_NETWORK):**
   ```
   docker compose -p novirell -f docker-compose.prod.yml --env-file .env config
   # ожидается: networks.web.name=mas-net (external), image novirell-backend:prod, router/service novirell
   ```
7. **Поднять стек:**
   ```
   ...
   ```
8. **Верификация:** `curl -fsS https://novirell.shop/healthz` → `200`; `docker inspect novirell-api-1 --format '{{.State.Health.Status}}'` → `healthy`; mas-стек не затронут (`docker ps` — контейнеры `mas-*` живы).
